    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-800">Buat Jadwal Bus Baru</h2>
                <p class="mt-1 text-sm text-gray-600">
                    Isi formulir di bawah untuk membuat jadwal keberangkatan baru
                </p>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input type="number" name="price_per_seat" id="price_per_seat"
                                value="{{ old('price_per_seat') }}" min="10000" step="1000"
                    ...
                    <div>
                        ...
                        <input type="number" name="available_seats" id="available_seats"
                            value="{{ old('available_seats') }}" min="1" max="100"
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const departureDefault = now.toISOString().slice(0, 16);
            if (!departureInput.value) {
                departureInput.value = departureDefault;
            }

            // Set arrival time to 4 hours after departure
            const arrivalDefault = new Date(now.getTime() + 4 * 60 * 60 * 1000).toISOString().slice(0, 16);
            if (!arrivalInput.value) {
                arrivalInput.value = arrivalDefault;
            }

            // Auto-set available seats based on selected bus
            ...
            busSelect.addEventListener('change', function() {
                ...
                if (selectedOption.value) {
                    const seatMatch = selectedOption.text.match(/(\d+) kursi/);
                    if (seatMatch) {
                        seatsInput.value = seatMatch[1];
                        seatsInput.max = seatMatch[1];
                    }
                }
            });
        });
