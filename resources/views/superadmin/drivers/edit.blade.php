...
<div class="container mx-auto px-4">
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-2xl font-semibold text-gray-800">Edit Driver</h1>
        <a href="{{ route('superadmin.drivers.index') }}" class="text-gray-600 hover:text-gray-900">Kembali</a>
    </div>

    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <form action="{{ route('superadmin.drivers.update', $driver->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                @if($driver->picture_url)
                    <div class="mb-3">
                        <img src="{{ $driver->picture_url }}" alt="{{ $driver->name }}" class="w-32 h-32 object-cover rounded-lg border border-gray-300">
                    </div>
                @endif
                ...
                <p class="text-gray-500 text-xs mt-1">Format: JPG, PNG, WEBP. Maks 2MB. Kosongkan jika tidak ingin mengganti.</p>
                @error('picture')
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('name') border-red-500 @enderror" 
                    id="name" name="name" type="text" placeholder="Masukkan nama driver" value="{{ old('name', $driver->name) }}" required>
                @error('name')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('phone') border-red-500 @enderror" 
                    id="phone" name="phone" type="text" placeholder="Misal: 08123456789" value="{{ old('phone', $driver->phone) }}" required>
                @error('phone')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('license_number') border-red-500 @enderror" 
                    id="license_number" name="license_number" type="text" placeholder="Masukkan nomor SIM driver" value="{{ old('license_number', $driver->license_number) }}">
